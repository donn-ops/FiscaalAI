      ...
      <View style={s.body}>
        ...
        <FlatList
          ...
          renderItem={({item})=>{
            ...
            return(
              <View style={[s.row,{borderColor:item.status==='action'||item.status==='overdue'?c.border:C.border}]}>
                <View style={s.dateBlock}>
                  <Text style={s.dateDay}>{item.day}</Text>
                  <Text style={s.dateMonth}>{item.month}</Text>
                </View>
...
const s=StyleSheet.create({
  ...
  filterBtn:{flex:1,backgroundColor:C.surface,borderRadius:999,paddingVertical:9,alignItems:'center',borderWidth:1,borderColor:C.border,...sh()},
