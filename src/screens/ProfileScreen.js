...
const SITUATIONS=[
  {key:'particulier',label:'Particulier'},{key:'zzp',label:"ZZP'er"},
  {key:'mkb',label:'MKB / BV'},{key:'dga',label:'DGA'},
];
...
export default function ProfileScreen({navigation}){
  ...
  const loadData=async()=>{
    ...
    setUserData(user);
    if(profile){setSituation(profile.situation||'particulier');setOwnHome(profile.ownHome||false);setHasPartner(profile.hasPartner||false);setHasKids(profile.hasKids||false);setNotifs(profile.notifications!==false);}
  };

  const saveProfile=async()=>{
    const profile={situation,ownHome,hasPartner,hasKids,notifications:notifs};
    await saveTaxProfile(profile);
    if(notifs){const granted=await requestPermissions();if(granted)await scheduleDeadlineNotifications(profile);}
